...
#Needs cscfeature and cscfeature_network variables
...
if exist "$omc_decoder"; then
    ...
    if exist /apex && ! grep -q "com.android.art" "$di_apex_list"; then
       abort "WARNING: You cant use OMC_DECODER if you dont have ALL DALVIKVM APEXs already mounted${n}Please enable setdefault apex_mount"
    fi
else
...
#Making & decrypting if needed cscfeature_network.xml
if exist "$omc_decoder"; then
    if ! run_jar "$omc_decoder" -d -i "$native_network" -o "$native_network"; then echo2 "CANT DECODE: $native_network"; fi
fi
content=$(string extract '<FeatureSet>' '</FeatureSet>' -f "$native_network")
if defined content && exist file "$cscfeature_network"; then replace "$content" "$(cat "$cscfeature_network")" "$native_network"; fi
